...
export function getNetworkInfo(port: number) {
  const interfaces = os.networkInterfaces();
  const addresses: { name: string; ip: string; url: string }[] = [];
...
export function troubleshootConnection(port: number, addresses: { ip: string }[]) {
